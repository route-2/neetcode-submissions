...
public:
    ...
    vector<int> getNewsFeed(int userId) {
        ...
        //
        // Conceptually, you can ignore the first optimization and just do the else
        // branch always; the REAL core logic is the final while-loop which merges.
        //
        // NOTE: This optimization branch is tricky and can be easy to get wrong.
        // I’m keeping comments so you know what it *intends* to do.
        // --------------------------------------------------------------------
        if (followMap[userId].size() >= 10) {
            ...
            for (auto f : followMap[userId]) {
                ...
                // WARNING: With this exact structure (maxHeap + -time),
                // popping here removes the current "largest" entry.
                // If "largest" corresponds to newest, this would drop newest
                // instead of oldest. So this optimization is easy to mess up.
                // (Many clean solutions just skip this whole if-branch.)
