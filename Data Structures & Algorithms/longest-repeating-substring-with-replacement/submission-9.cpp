...
When the window became too big (needed > k replacements), we moved left.
But moving left means the character at left is no longer inside the window, so:

✅ we must do cnt[s[left]]-- to keep the counts accurate for the new window.
        */
    }
};
